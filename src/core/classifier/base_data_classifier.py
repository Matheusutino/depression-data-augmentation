...
class MILDataClassifier(BaseDataClassifier):
...
class SILDataClassifier(BaseDataClassifier):
    ...
    def train_and_evaluate(self, X, y, x_val, x_test, y_val, y_test, n_trials=50, pooling_type='average'):
        """Treina o modelo XGBoost utilizando Optuna para otimização dos hiperparâmetros"""
        
        # Preparar os dados no formato SIL
        X_flattened, y_flattened = self.prepare_data(X, y)
        
        # Gerar embeddings para todos os posts
        X_embeds = [self.generate_embeddings(posts) for posts in X]
        x_val_embeds = [self.generate_embeddings(posts) for posts in x_val]
        x_test_embeds = [self.generate_embeddings(posts) for posts in x_test]
        
        # Aplicar pooling
        X_pool = np.array([self.apply_pooling(embeds, pooling_type) for embeds in X_embeds])
        x_val_pool = np.array([self.apply_pooling(embeds, pooling_type) for embeds in x_val_embeds])
        x_test_pool = np.array([self.apply_pooling(embeds, pooling_type) for embeds in x_test_embeds])
        ...
        # Otimizar os hiperparâmetros usando Optuna
        best_params, trials_history = self.optimize_hyperparameters(X_pool, y_flattened, x_val_pool, y_val, n_trials)
        
        # Treinar o modelo final com os melhores parâmetros
        dtrain = xgb.DMatrix(X_pool, label=y_flattened)
        dtest = xgb.DMatrix(x_test_pool, label=y_test)
