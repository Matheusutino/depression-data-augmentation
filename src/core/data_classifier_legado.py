...
class DataClassifier:
    def __init__(self, embed_name, random_seed=42):
        """Inicializa a classe com o nome do modelo de embedding e uma seed"""
        self.embed_name = embed_name
        self.model = SentenceTransformer(embed_name)
        self.scaler = StandardScaler()
        self.random_seed = random_seed
        # Setar a seed para reproducibilidade
    ...
    def objective(self, trial, x_train_pool, y_train, x_val_pool, y_val, device = "cuda"):
        """Função objetivo para o Optuna"""
        
        # Definir os parâmetros do modelo a serem otimizados
        eta = trial.suggest_loguniform('eta', 0.01, 0.3)  # Log uniform distribution for eta
        gamma = trial.suggest_uniform('gamma', 0, 0.2)  # Uniform distribution for gamma
        max_depth = trial.suggest_int('max_depth', 3, 10)  # Integer for max_depth
        subsample = trial.suggest_uniform('subsample', 0.8, 1.0)  # Uniform distribution for subsample
        min_child_weight = trial.suggest_int('min_child_weight', 1, 5)  # Integer for min_child_weight
        lambda_ = trial.suggest_uniform('lambda', 0, 10.0)  # Uniform distribution for lambda
        alpha = trial.suggest_uniform('alpha', 0, 10.0)  # Uniform distribution for alpha
        ...
        params = {
            ...
            'lambda': lambda_,
            'alpha': alpha,
            'device': device,  # Usando GPU para treinamento
            'predictor': 'gpu_predictor',
            'seed': self.random_seed  # Fixar a semente para reprodutibilidade
        }
        ...
        return f1  # A função objetivo minimiza ou maximiza esse valor
